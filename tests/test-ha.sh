...
distfs --disable-doh --allow-insecure --use-pinentry=false --timeline-sample-rate=1.0 --config "$DISTFS_CONFIG_DIR/config.json" put /tmp/ha.txt /ha/ha-test.bin
...
distfs --disable-doh --allow-insecure --use-pinentry=false --timeline-sample-rate=1.0 --config "$DISTFS_CONFIG_DIR/config.json" whoami > /dev/null

echo "Verifying timeline consistency across remaining nodes..."
distfs --disable-doh --allow-insecure --use-pinentry=false --timeline-sample-rate=1.0 --config "$DISTFS_CONFIG_DIR/config.json" verify-timeline

echo "Verifying file is STILL readable from remaining nodes..."
distfs --disable-doh --allow-insecure --use-pinentry=false --timeline-sample-rate=1.0 --config "$DISTFS_CONFIG_DIR/config.json" get /ha/ha-test.bin /tmp/ha-back.txt
