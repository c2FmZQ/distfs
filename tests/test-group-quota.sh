...
distfs -disable-doh -allow-insecure -use-pinentry=false -config "$CONFIG" mkdir /users/quota-user/group-dir
distfs -disable-doh -allow-insecure -use-pinentry=false -config "$CONFIG" chgrp "$G_ID" /users/quota-user/group-dir
distfs -disable-doh -allow-insecure -use-pinentry=false -config "$CONFIG" chmod 0770 /users/quota-user/group-dir

echo "Alice: Uploading 1st file to group-owned directory..."
echo "hello world" > /tmp/file1.txt
distfs -disable-doh -allow-insecure -use-pinentry=false -config "$CONFIG" put /tmp/file1.txt /users/quota-user/group-dir/file1.txt
...
distfs -disable-doh -allow-insecure -use-pinentry=false -admin -config "$DISTFS_CONFIG_DIR/config.json" admin-group-quota "$G_ID" 1000 2

echo "Alice: Uploading 2nd file to group-owned directory (should fail: Group Inode Quota 2+1 > 2)..."
echo "overflow" > /tmp/file2.txt
if distfs -disable-doh -allow-insecure -use-pinentry=false -config "$CONFIG" put /tmp/file2.txt /users/quota-user/group-dir/file2.txt 2>/dev/null; then
...
distfs -disable-doh -allow-insecure -use-pinentry=false -admin -config "$DISTFS_CONFIG_DIR/config.json" admin-group-quota "$G_ID" 10 3
...
ALICE_ID=$(distfs -disable-doh -allow-insecure -use-pinentry=false -config /tmp/quota-user-config.json whoami)
distfs -disable-doh -allow-insecure -use-pinentry=false -admin -config "$DISTFS_CONFIG_DIR/config.json" admin-user-quota "$ALICE_ID" 1000 2
...
distfs -disable-doh -allow-insecure -use-pinentry=false -config "$CONFIG" put /tmp/personal.txt /users/quota-user/personal.txt
echo "PASS: Alice uploaded 1st personal file (2/2)"

echo "Alice: Uploading 3rd file (should fail: User Inode Quota 2+1 > 2)..."
if distfs -disable-doh -allow-insecure -use-pinentry=false -config "$CONFIG" put /tmp/file2.txt /users/quota-user/personal2.txt 2>/dev/null; then
