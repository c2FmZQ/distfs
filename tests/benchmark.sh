...
distfs-bench --config "$DISTFS_CONFIG_DIR/config.json" --mode mkdir --count 100 --path /bench-workspace

echo "Measuring Data Throughput (1MB Chunks)..."
distfs-bench --config "$DISTFS_CONFIG_DIR/config.json" --mode put --size 1048576 --count 10 --path /bench-workspace
