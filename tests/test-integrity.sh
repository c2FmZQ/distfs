...
distfs --disable-doh --allow-insecure --use-pinentry=false --timeline-sample-rate=1.0 --config "$CONFIG" put /tmp/crit.txt /users/integrity-user/critical.txt
...
distfs --disable-doh --allow-insecure --use-pinentry=false --timeline-sample-rate=1.0 --config "$CONFIG" get /users/integrity-user/critical.txt /tmp/crit-back.txt
