...
G_OUT=$(distfs -disable-doh -use-pinentry=false -config /tmp/alice.json group-create exchange-test)
G_ID=$(echo "$G_OUT" | grep "^ID:" | awk '{print $2}')
echo "Group ID: $G_ID"

echo "Alice: Adding Bob via contact string (with -f)..."
distfs -disable-doh -use-pinentry=false -config /tmp/alice.json group-add -f "$G_ID" "$BOB_CONTACT" "Bob (OOB)"

echo "Alice: Verifying Bob is in member list..."
if distfs -disable-doh -use-pinentry=false -config /tmp/alice.json group-members "$G_ID" | grep -q "Bob (OOB)"; then
...
echo "y" | distfs -disable-doh -use-pinentry=false -config /tmp/alice.json group-add "$G_ID" "$CAROL_CONTACT" "Carol (Interactive)"

echo "Alice: Verifying Carol is in member list..."
if distfs -disable-doh -use-pinentry=false -config /tmp/alice.json group-members "$G_ID" | grep -q "Carol (Interactive)"; then
