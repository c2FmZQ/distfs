...
distfs -config /tmp/u1-writable.json init -meta http://storage-node-1:8080 -id user1-writable@example.com
JWT1=$(wget -qO- "http://test-auth:8080/mint?email=user1-writable@example.com")
distfs -config /tmp/u1-writable.json register -jwt "$JWT1"

echo "Initializing user2 (Collaborator)..."
distfs -config /tmp/u2-writable.json init -meta http://storage-node-1:8080 -id user2-writable@example.com
JWT2=$(wget -qO- "http://test-auth:8080/mint?email=user2-writable@example.com")
distfs -config /tmp/u2-writable.json register -jwt "$JWT2"

echo "User 1: Creating world-writable directory..."
distfs -config /tmp/u1-writable.json mkdir /shared
distfs -config /tmp/u1-writable.json chmod 0777 /shared

echo "User 2: Creating a file in shared directory..."
echo "user2 data" > /tmp/u2.txt
distfs -config /tmp/u2-writable.json put /tmp/u2.txt /shared/user2-file.txt
distfs -config /tmp/u2-writable.json chmod 0644 /shared/user2-file.txt

echo "User 1: Verifying file from user2..."
distfs -config /tmp/u1-writable.json get /shared/user2-file.txt /tmp/u1-back.txt
...
distfs -config /tmp/u1-writable.json put /tmp/shared-file.txt /shared/shared-file.txt
distfs -config /tmp/u1-writable.json chmod 0666 /shared/shared-file.txt

echo "User 2: Overwriting shared file..."
echo "overwritten by user2" > /tmp/u2-overwrite.txt
distfs -config /tmp/u2-writable.json put /tmp/u2-overwrite.txt /shared/shared-file.txt

echo "User 1: Verifying overwrite..."
distfs -config /tmp/u1-writable.json get /shared/shared-file.txt /tmp/u1-overwrite-back.txt
