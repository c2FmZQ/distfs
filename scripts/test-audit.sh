...
distfs -disable-doh -use-pinentry=false -config "$DISTFS_CONFIG_DIR/config.json" mkdir /audit-test
distfs -disable-doh -use-pinentry=false -config "$DISTFS_CONFIG_DIR/config.json" mkdir /audit-test/subdir1
distfs -disable-doh -use-pinentry=false -config "$DISTFS_CONFIG_DIR/config.json" mkdir /audit-test/subdir2
...
distfs -disable-doh -use-pinentry=false -admin -config "$DISTFS_CONFIG_DIR/config.json" admin-audit > /tmp/audit.out
cat /tmp/audit.out
