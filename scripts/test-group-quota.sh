...
distfs -disable-doh -use-pinentry=false -config /tmp/alice-quota.json init --new --jwt "$ALICE_JWT" --server http://storage-node-1:8080
...
distfs -disable-doh -use-pinentry=false -config /tmp/alice-quota.json group-create --quota quota-test > /tmp/group-out.txt
...
distfs -disable-doh -use-pinentry=false -config /root/.distfs/config.json mkdir /quota-dir
distfs -disable-doh -use-pinentry=false -config /root/.distfs/config.json admin-chown -f ":$G_ID" /quota-dir
distfs -disable-doh -use-pinentry=false -config /root/.distfs/config.json chmod 0770 /quota-dir
# Wait for directory to be visible to Alice
echo "Waiting for /quota-dir to be visible to Alice..."
MAX_WAIT=20
COUNT=0
until distfs -disable-doh -use-pinentry=false -config /tmp/alice-quota.json ls /quota-dir >/dev/null 2>/tmp/ls-err.txt; do 
    sleep 1
    COUNT=$((COUNT + 1))
    if [ $COUNT -ge $MAX_WAIT ]; then
        echo "FAIL: /quota-dir never became visible to Alice"
        echo "Last error: $(cat /tmp/ls-err.txt)"
        distfs -disable-doh -use-pinentry=false -config /tmp/alice-quota.json ls /
        exit 1
    fi
done
...
if ! timeout 30s distfs -disable-doh -use-pinentry=false -config /tmp/alice-quota.json put /tmp/file1.txt /quota-dir/file1.txt; then
...
distfs -disable-doh -use-pinentry=false -config /root/.distfs/config.json admin-group-quota "$G_ID" 1000 2

echo "Alice: Uploading 2nd file to group-owned directory (should fail: Group Inode Quota 2+1 > 2)..."
echo "overflow" > /tmp/file2.txt
if distfs -disable-doh -use-pinentry=false -config /tmp/alice-quota.json put /tmp/file2.txt /quota-dir/file2.txt 2>/dev/null; then
...
distfs -disable-doh -use-pinentry=false -config /root/.distfs/config.json admin-group-quota "$G_ID" 10 3
...
distfs -disable-doh -use-pinentry=false -config /root/.distfs/config.json admin-user-quota alice-quota@example.com 1000 2

echo "Admin: Provisioning test dir for Alice..."
TEST_DIR="/alice-dir-$(date +%s)"
distfs -disable-doh -use-pinentry=false -config /root/.distfs/config.json mkdir "$TEST_DIR"
distfs -disable-doh -use-pinentry=false -config /root/.distfs/config.json admin-chown -f alice-quota@example.com "$TEST_DIR"
# Wait for directory to be visible to Alice
until distfs -disable-doh -use-pinentry=false -config /tmp/alice-quota.json ls "$TEST_DIR" >/dev/null 2>&1; do sleep 0.5; done

echo "Alice: Uploading 2nd file (should succeed - usage 1->2 <= 2)..."
echo "personal file" > /tmp/personal.txt
if ! distfs -disable-doh -use-pinentry=false -config /tmp/alice-quota.json put /tmp/personal.txt "$TEST_DIR/personal.txt"; then
...
if distfs -disable-doh -use-pinentry=false -config /tmp/alice-quota.json put /tmp/file2.txt "$TEST_DIR/file2.txt" 2>/dev/null; then
