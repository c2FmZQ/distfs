...
U1_OUT=$(distfs -disable-doh -use-pinentry=false -config /tmp/u1-group.json init --new -server http://storage-node-1:8080 -jwt "$JWT1")
echo "$U1_OUT"
...
U2_OUT=$(distfs -disable-doh -use-pinentry=false -config /tmp/u2-group.json init --new -server http://storage-node-1:8080 -jwt "$JWT2")
echo "$U2_OUT"
...
U3_OUT=$(distfs -disable-doh -use-pinentry=false -config /tmp/u3-group.json init --new -server http://storage-node-1:8080 -jwt "$JWT3")
echo "$U3_OUT"
U3_ID=$(echo "$U3_OUT" | grep "User ID:" | cut -d: -f2 | tr -d ' ')
echo "User 3 ID: $U3_ID"

echo "User 1: Creating group 'project-x'..."
G1_OUT=$(distfs -disable-doh -use-pinentry=false -config /tmp/u1-group.json group-create project-x)
echo "$G1_OUT"
...
distfs -disable-doh -use-pinentry=false -config /tmp/u1-group.json group-add "$G1_ID" "$U2_ID"

echo "Admin: Creating directory and assigning to user1..."
distfs -disable-doh -use-pinentry=false -config /root/.distfs/config.json mkdir /group-shared
sleep 2
echo "y" | distfs -disable-doh -use-pinentry=false -admin -config /root/.distfs/config.json admin-chown "$U1_ID" /group-shared

echo "User 1: Assigning directory to group..."
distfs -disable-doh -use-pinentry=false -config /tmp/u1-group.json chgrp "$G1_ID" /group-shared
distfs -disable-doh -use-pinentry=false -config /tmp/u1-group.json chmod 0770 /group-shared

echo "User 1: Uploading file to group-shared..."
echo "top secret project info" > /tmp/project.txt
distfs -disable-doh -use-pinentry=false -config /tmp/u1-group.json put /tmp/project.txt /group-shared/plan.txt
distfs -disable-doh -use-pinentry=false -config /tmp/u1-group.json chmod 0660 /group-shared/plan.txt

echo "User 2 (Member): Attempting to read file..."
distfs -disable-doh -use-pinentry=false -config /tmp/u2-group.json get /group-shared/plan.txt /tmp/u2-plan.txt
...
if distfs -disable-doh -use-pinentry=false -config /tmp/u3-group.json get /group-shared/plan.txt /tmp/u3-fail.txt 2>/dev/null; then
...
distfs -disable-doh -use-pinentry=false -config /tmp/u2-group.json put /tmp/u2-update.txt /group-shared/plan.txt

echo "User 1 (Owner): Verifying member's update..."
distfs -disable-doh -use-pinentry=false -config /tmp/u1-group.json get /group-shared/plan.txt /tmp/u1-verify.txt
...
if distfs -disable-doh -use-pinentry=false -config /tmp/u2-group.json group-list | grep -q "project-x"; then
...
distfs -disable-doh -use-pinentry=false -config /tmp/u1-group.json group-remove "$G1_ID" "$U2_ID"
...
if distfs -disable-doh -use-pinentry=false -config /tmp/u2-group.json get /group-shared/plan.txt /tmp/u2-fail.txt 2>/dev/null; then
