...
distfs -config /tmp/u1-group.json init -meta http://storage-node-1:8080 -id user1-group@example.com
JWT1=$(wget -qO- "http://test-auth:8080/mint?email=user1-group@example.com")
U1_OUT=$(distfs -config /tmp/u1-group.json register -jwt "$JWT1")
echo "$U1_OUT"
U1_ID=$(echo "$U1_OUT" | grep "ID:" | cut -d: -f2 | tr -d ' ')
echo "User 1 ID: $U1_ID"

echo "Initializing user2 (Group Member)..."
distfs -config /tmp/u2-group.json init -meta http://storage-node-1:8080 -id user2-group@example.com
JWT2=$(wget -qO- "http://test-auth:8080/mint?email=user2-group@example.com")
U2_OUT=$(distfs -config /tmp/u2-group.json register -jwt "$JWT2")
echo "$U2_OUT"
U2_ID=$(echo "$U2_OUT" | grep "ID:" | cut -d: -f2 | tr -d ' ')
echo "User 2 ID: $U2_ID"

echo "Initializing user3 (Non-Member)..."
distfs -config /tmp/u3-group.json init -meta http://storage-node-1:8080 -id user3-group@example.com
JWT3=$(wget -qO- "http://test-auth:8080/mint?email=user3-group@example.com")
U3_OUT=$(distfs -config /tmp/u3-group.json register -jwt "$JWT3")
echo "$U3_OUT"
U3_ID=$(echo "$U3_OUT" | grep "ID:" | cut -d: -f2 | tr -d ' ')
echo "User 3 ID: $U3_ID"

echo "User 1: Creating group 'project-x'..."
G1_OUT=$(distfs -config /tmp/u1-group.json group-create project-x)
echo "$G1_OUT"
...
distfs -config /tmp/u1-group.json group-add "$G1_ID" "$U2_ID"

echo "User 1: Creating directory and assigning to group..."
distfs -config /tmp/u1-group.json mkdir /group-shared
distfs -config /tmp/u1-group.json chgrp "$G1_ID" /group-shared
distfs -config /tmp/u1-group.json chmod 0770 /group-shared

echo "User 1: Uploading file to group-shared..."
echo "top secret project info" > /tmp/project.txt
distfs -config /tmp/u1-group.json put /tmp/project.txt /group-shared/plan.txt
distfs -config /tmp/u1-group.json chmod 0660 /group-shared/plan.txt

echo "User 2 (Member): Attempting to read file..."
distfs -config /tmp/u2-group.json get /group-shared/plan.txt /tmp/u2-plan.txt
...
if distfs -config /tmp/u3-group.json get /group-shared/plan.txt /tmp/u3-fail.txt 2>/dev/null; then
...
distfs -config /tmp/u2-group.json put /tmp/u2-update.txt /group-shared/plan.txt

echo "User 1 (Owner): Verifying member's update..."
distfs -config /tmp/u1-group.json get /group-shared/plan.txt /tmp/u1-verify.txt
