...
distfs -disable-doh -use-pinentry=false -config "$CONFIG" put /tmp/trash.txt /users/gc-user/trash.txt

echo "Deleting file..."
distfs -disable-doh -use-pinentry=false -config "$CONFIG" rm /users/gc-user/trash.txt

echo "Waiting for background GC..."
# Verify authentication
distfs -disable-doh -use-pinentry=false -config "$CONFIG" whoami > /dev/null
