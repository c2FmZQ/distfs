...
# Bob runs init
distfs -disable-doh -use-pinentry=false -config "$BOB_DIR/config.json" init -server "http://storage-node-1:8080" -jwt "$BOB_JWT" -new
...
if distfs -disable-doh -use-pinentry=false -config "$BOB_DIR/config.json" put /tmp/bob.txt /users/bob.txt 2>/dev/null; then
...
echo "y" | distfs -disable-doh -use-pinentry=false -admin -config "$DISTFS_CONFIG_DIR/config.json" registry-add --unlock --quota 1000000,5000 --home bob-reg bob-registry@example.com

# 4. Verify Bob can now access his home directory
echo "4. Verifying Bob's unlocked access..."
if distfs -disable-doh -use-pinentry=false -config "$BOB_DIR/config.json" put /tmp/bob.txt /users/bob-reg/mydata.txt; then
...
distfs -disable-doh -use-pinentry=false -admin -config "$DISTFS_CONFIG_DIR/config.json" ls -l /registry > /tmp/reg-ls.txt
