...
/bin/distfs-bench -server "$SERVER_URL" -jwt "$JWT" -mode mkdir -workers 5 -count 100

# 2. Small File Throughput (1KB)
echo "Benchmarking Small File Throughput (1KB)..."
/bin/distfs-bench -server "$SERVER_URL" -jwt "$JWT" -mode put -workers 5 -count 10 -size 1024

# 3. Large File Throughput (5MB)
echo "Benchmarking Large File Throughput (5MB)..."
/bin/distfs-bench -server "$SERVER_URL" -jwt "$JWT" -mode put -workers 2 -count 20 -size 5242880

# 4. Read Latency & Throughput (5MB)
echo "Benchmarking Read Performance (5MB)..."
/bin/distfs-bench -server "$SERVER_URL" -jwt "$JWT" -mode get -workers 2 -count 20 -size 5242880
