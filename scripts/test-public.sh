...
distfs -config /tmp/u1.json init -meta http://storage-node-1:8080 -id user1@example.com
JWT1=$(wget -qO- "http://test-auth:8080/mint?email=user1@example.com")
distfs -config /tmp/u1.json register -jwt "$JWT1"

echo "Initializing Reader (user2)..."
distfs -config /tmp/u2.json init -meta http://storage-node-1:8080 -id user2@example.com
JWT2=$(wget -qO- "http://test-auth:8080/mint?email=user2@example.com")
distfs -config /tmp/u2.json register -jwt "$JWT2"

echo "User 1: Creating public test dir..."
until distfs -config /tmp/u1.json mkdir /public; do
    echo "Retrying public mkdir..."
    sleep 1
done

echo "User 1: Making directory public (chmod 0755)..."
distfs -config /tmp/u1.json chmod 0755 /public

echo "User 1: Uploading private file..."
echo "secret message" > /tmp/secret.txt
distfs -config /tmp/u1.json put /tmp/secret.txt /public/test.txt

echo "User 2: Attempting to read (should fail initially)..."
if distfs -config /tmp/u2.json get /public/test.txt /tmp/fail.txt 2>/dev/null; then
...
distfs -config /tmp/u1.json chmod 0644 /public/test.txt

echo "User 2: Attempting to read public file..."
distfs -config /tmp/u2.json get /public/test.txt /tmp/success.txt
...
distfs -config /tmp/u1.json chmod 0600 /public/test.txt

echo "User 2: Attempting to read (should fail again)..."
if distfs -config /tmp/u2.json get /public/test.txt /tmp/fail2.txt 2>/dev/null; then
