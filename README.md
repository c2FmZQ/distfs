...
**The Mechanism:** DistFS bakes NIST-standardized PQC directly into the application layer. Every Remote Procedure Call (RPC) mutation is constructed as a **Sealed Request**. The client queries the cluster for its active, periodically rotating ML-KEM-768 (Crystals-Kyber) Epoch Key. The client encapsulates a shared secret, encrypts the entire JSON RPC payload using AES-256-GCM, and cryptographically signs the envelope using its own ML-DSA (Dilithium) identity key. 
...
**The Mechanism:** DistFS strips the metadata server of this authority. An Inode's ID is not a server-assigned UUID; it is a mathematical commitment generated by the client: `Inode ID = hex(SHA256(OwnerID || "|" || Nonce))[:32]`. 
Furthermore, delegating write access (e.g., via groups or ACLs) requires a cryptographic `OwnerDelegationSig`—a signature from the true Owner over a hash of the delegation state.
