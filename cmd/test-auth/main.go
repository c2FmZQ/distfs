...
func main() {
	cmd := &cli.Command{
		Name:  "test-auth",
		Usage: "Mock OIDC Server for DistFS testing",
		Flags: []cli.Flag{
			&cli.StringFlag{Name: "addr", Value: ":8080", Usage: "Address to listen on"},
			&cli.StringFlag{Name: "baseURL", Value: "", Usage: "Base URL for OIDC endpoints (e.g. https://web-test-server/auth)"},
		},
		Action: func(ctx context.Context, cmd *cli.Command) error {
			addr := cmd.String("addr")
			base := cmd.String("baseURL")
